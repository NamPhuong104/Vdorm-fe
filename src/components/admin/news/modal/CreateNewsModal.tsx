...
const CreateNewsModal = (props: IProps) => {
  const {
    isCreateModalOpen,
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      setIsLoading(true);
      const res = await axiosAuth.post(`/news`, data);
  ...
  return (
    <Modal
      ...
      style={{ margin: '30px 0' }}
    >
      <Form name="create-news" form={form} onFinish={onFinish}>
        <Flex gap={'20px'}>
          <div style={{ width: '50%' }}>
            <Form.Item<INew>
              label="Tiêu đề"
