...
const StatisticService = () => {
  ...
  const [serviceList, setServiceList] = useState<
    { month: string; year: string; paid: number; unpaid: number }[]
  >([]);
  ...
  const fetchServiceList = async (queryString?: string) => {
    if (queryString && queryString !== '') {
      try {
        setIsLoading(true);
        const res = await axiosAuth.get(
          `/invoice-details/statistic/service?${queryString}`,
        );
        setServiceList(res?.data?.data?.result);
        setIsLoading(false);
      } catch (error: any) {
        setServiceList([]);
        setIsLoading(false);
      ...
      try {
        setIsLoading(true);
        const res = await axiosAuth.get(`/invoice-details/statistic/service`);
        setServiceList(res?.data?.data?.result);
        setIsLoading(false);
      } catch (error: any) {
        setServiceList([]);
  ...
  const columns: ColumnsType<{
    ...
    },
    {
      ...
      ellipsis: true,
      render: (_, record) => {
        return currencyFormat(record.paid);
      },
    },
    {
      title: 'Chưa thu (VND)',
      ...
      ellipsis: true,
      render: (_, record) => {
        return currencyFormat(record.unpaid);
      },
    },
    {
      title: 'Phải thu (VND)',
      ...
      ellipsis: true,
      render: (_, record) => {
        return currencyFormat(record.paid + record.unpaid);
      },
    },
  ];

  return (
    <div className="statistic-service-page">
      <AppTitle
        moduleName="Tiền Dịch Vụ"
        isDisableExportStatistic={isDisableExportStatistic}
        exportStatisticLink={`${
          process.env.NEXT_PUBLIC_BACKEND_URL
        }/invoice-details/statistic/service/export?${buildQueryString(
