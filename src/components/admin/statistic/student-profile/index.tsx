...
const StatisticStudentProfile = () => {
  const { status, data: session } = useSession();
  const axiosAuth = useAxiosAuth();
  const [branchList, setBranchList] = useState<IBranchOption[]>([]);
  const [studentStatisticList, setStudentStatisticList] = useState<
    {
      month: string;
      year: string;
      totalStaying: number;
      totalStopped: number;
      totalStoppedViolation: number;
    }[]
  >([]);
  ...
  const fetchStudentStatisticList = async (queryString?: string) => {
    if (queryString && queryString !== '') {
      try {
        setIsLoading(true);
        const res = await axiosAuth.get(
          `/student-profiles/statistic/student?${queryString}`,
        );
        setStudentStatisticList(res?.data?.data?.result);
        setIsLoading(false);
      } catch (error: any) {
        setStudentStatisticList([]);
        setIsLoading(false);
        notification.error({
      ...
      try {
        setIsLoading(true);
        const res = await axiosAuth.get(`/student-profiles/statistic/student`);
        setStudentStatisticList(res?.data?.data?.result);
        setIsLoading(false);
      } catch (error: any) {
        setStudentStatisticList([]);
  ...
  const columns: ColumnsType<{
    ...
    {
      title: 'Đang ở',
      dataIndex: 'totalStaying',
      key: 'totalStaying',
      align: 'center',
      width: '25%',
      ellipsis: true,
      render: (_, record) => {
        return currencyFormat(record?.totalStaying);
      },
    },
    {
      title: 'Ngừng ở',
      dataIndex: 'totalStopped',
      key: 'totalStopped',
      align: 'center',
      width: '25%',
      ellipsis: true,
      render: (_, record) => {
        return currencyFormat(record?.totalStopped);
      },
    },
    {
      title: 'Ngừng ở do vi phạm',
      dataIndex: 'totalStoppedViolation',
      key: 'totalStoppedViolation',
      align: 'center',
      width: '30%',
      ellipsis: true,
      render: (_, record) => {
        return currencyFormat(record?.totalStoppedViolation);
      },
    },
  ];

  return (
    <div className="statistic-student-profile">
      <AppTitle
        moduleName="Số Lượng Sinh Viên"
        isDisableExportStatistic={isDisableExportStatistic}
        exportStatisticLink={`${
          process.env.NEXT_PUBLIC_BACKEND_URL
        }/student-profiles/statistic/student/export?${buildQueryString(
      ...
      <Table
        columns={columns}
        dataSource={studentStatisticList}
        rowKey={'_id'}
        loading={isLoading}
        scroll={{ x: 'auto' }}
        pagination={{
          total: studentStatisticList.length,
...
export default StatisticStudentProfile;
