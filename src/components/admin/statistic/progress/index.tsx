...
const StatisticProgress = () => {
  ...
  const [progressList, setProgressList] = useState<
    { month: string; year: string; progress: number }[]
  >([]);
  ...
  const fetchProgressList = async (queryString?: string) => {
    if (queryString && queryString !== '') {
      try {
        setIsLoading(true);
        const res = await axiosAuth.get(
          `/invoice-details/statistic/progress?${queryString}`,
        );
        setProgressList(res?.data?.data?.result);
        setIsLoading(false);
      } catch (error: any) {
        setProgressList([]);
        setIsLoading(false);
      ...
      try {
        setIsLoading(true);
        const res = await axiosAuth.get(`/invoice-details/statistic/progress`);
        setProgressList(res?.data?.data?.result);
        setIsLoading(false);
      } catch (error: any) {
        setProgressList([]);
  ...
  return (
    <div className="statistic-progress-page">
      <AppTitle
        moduleName="Tiến Độ Thanh Toán Hóa Đơn"
        isDisableExportStatistic={isDisableExportStatistic}
        exportStatisticLink={`${
          process.env.NEXT_PUBLIC_BACKEND_URL
        }/invoice-details/statistic/progress/export?${buildQueryString(
