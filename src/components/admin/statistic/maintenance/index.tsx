...
const StatisticMaintenance = () => {
  const { status, data: session } = useSession();
  const axiosAuth = useAxiosAuth();
  const [branchList, setBranchList] = useState<IBranchOption[]>([]);
  const [maintenanceTurnList, setMaintenanceTurnList] = useState<
    {
      month: string;
      year: string;
      infrastructureQrCode: { name: string };
      turn: number;
    }[]
  >([]);
  ...
  const fetchMaintenanceTurnList = async (queryString?: string) => {
    if (queryString && queryString !== '') {
      try {
        setIsLoading(true);
        const res = await axiosAuth.get(
          `/maintenances/statistic/maintenance?${queryString}`,
        );
        setMaintenanceTurnList(res?.data?.data?.result);
        setIsLoading(false);
      } catch (error: any) {
        setMaintenanceTurnList([]);
        setIsLoading(false);
        notification.error({
      ...
      try {
        setIsLoading(true);
        const res = await axiosAuth.get(`/maintenances/statistic/maintenance`);
        setMaintenanceTurnList(res?.data?.data?.result);
        setIsLoading(false);
      } catch (error: any) {
        setMaintenanceTurnList([]);
  ...
  const columns: ColumnsType<{
    ...
    },
    {
      ...
      dataIndex: ['infrastructureQrCode', 'name'],
      key: 'infrastructureQrCode',
      align: 'center',
      width: '60%',
      ellipsis: true,
    },
    {
      ...
      ellipsis: true,
      render: (_, record) => {
        return currencyFormat(record.turn);
      },
    },
  ];

  return (
    <div className="statistic-maintenance-page">
      <AppTitle
        moduleName="Lượt Bảo Trì"
        isDisableExportStatistic={isDisableExportStatistic}
        exportStatisticLink={`${
          process.env.NEXT_PUBLIC_BACKEND_URL
        }/maintenances/statistic/maintenance/export?${buildQueryString(
      ...
      <Table
        columns={columns}
        dataSource={maintenanceTurnList}
        rowKey={'_id'}
        loading={isLoading}
        scroll={{ x: 'auto' }}
        pagination={{
          total: maintenanceTurnList.length,
...
export default StatisticMaintenance;
